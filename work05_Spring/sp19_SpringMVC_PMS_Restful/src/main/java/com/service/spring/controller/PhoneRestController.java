...
public class PhoneRestController {
	...
	//특정 폰을 가져오는 요청 
	/*
	@GetMapping 어노테이션에 특정한 파라미터 값을 {num}이라는 형태로 작성해야 한다.
	이 값을 받을 때는 @PathVariable 어노테이션으로 가져올 파라미터 값을 받아간다. 	
	*/
	...
	/*
	 ...
	 POST 요청에서는 추가되는 객체를 받을 때 @RequestBody를 사용한다. -> 넘겨주는 객체의 이름으로 지정해야 함.  
	*/
	@PostMapping("/phone")
	...
	public ResponseEntity delete(@PathVariable String num ) {
		try {
			...
			phoneService.delete(Arrays.asList(num));  //num을 List로 만든 것. 아니면 new List 생성해서 num을 넣어도 됨. -> 동적 쿼리로 작성해서 그런 것.
			return new ResponseEntity(HttpStatus.OK);
		} catch (Exception e) {
			return new ResponseEntity(HttpStatus.NO_CONTENT);
		}
